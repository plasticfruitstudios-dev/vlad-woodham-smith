...
# loops in public/videos/ (1280x960, 25fps, H.264, silent, yuv420p), and the
...
POSTER="public/videos/posters/${NAME}.jpg"
...
# poster = frame 0 of the web encode (the grid shows it until the loop plays)
ffmpeg -y -v error -i "$OUT" -frames:v 1 -q:v 4 "$POSTER"
cp "$POSTER" "src/assets/videos/posters/${NAME}.jpg"
...
echo ""
echo "Now add the card to src/data/grid-*.json:"
echo "  \"media\": { \"type\": \"video\", \"src\": \"videos/${NAME}.mp4\", \"poster\": \"videos/posters/${NAME}.jpg\" }"
